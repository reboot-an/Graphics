...
class Editor_PinningMergeJob():
    ...
    def get_job_definition(self, editor, agent, editor_pin_target_branch, editor_pin_ci_branch):
        ...
        commands = [
            ...
            f'pipenv run python3 -m .yamato.ruamel.merge_revisions --revision $GIT_REVISION --target-branch { editor_pin_target_branch }'
        ]
        
        # construct job
        job = YMLJob()
        job.set_name(f'Merge editor revisions to {editor_pin_ci_branch}')
        job.set_agent(agent)
        job.add_var_custom('CI', True)
        job.add_commands(commands)
        job.add_dependencies([f'{abv_filepath()}#{abv_job_id_all_project_ci(editor)}'])
        # job.add_trigger_integration_branch(editor_pin_ci_branch) # TODO uncomment
        return job